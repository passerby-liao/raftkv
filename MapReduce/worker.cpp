...
// 按单词相同的分到末尾reduce_idx相同的文件，方便reduce
void writeInDisk(const vector<KeyValue> &kvs, int mapTaskIdx)
{
    for (const auto &v : kvs)
    {
        ...
        int ret = access(path.c_str(), F_OK); // -1 文件不存在  0 已经存在
        if (ret == -1)
        {
            //  O_WRONLY 只写    O_CREAT 文件不存在则创建文件    O_APPEND 写入时追加写入
            // 第二个数字 6 表示文件所有者具有读写权限。第三个数字 6 表示文件所有者所在的组具有读写权限。第四个数字 4 表示其他用户只具有读权限。
...
int main()
{
    ...
    // RTLD_LAZY 需要时才解析共享库中的符号（函数、变量等）
    // RTLD_NOW 加载时就被解析
    ...
    cout << "pthread_mutex_destroy" << endl;
    pthread_cond_destroy(&cond);
    cout << "pthread_cond_destroy" << endl;
    return 0;
}